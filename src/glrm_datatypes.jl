...
impute(D::OrdinalDomain, l::Loss, u::Float64) = impute(OrdinalDomain(0,max_count), l, u)
...
function impute(domains::Array{Domain,1}, losses::Array{Loss,1}, U::Array{Float64,2})
	...
	for j in 1:n
		for i in 1:m
			A_imputed[i,j] = impute(domains[j], losses[j], A[i,j]) # tests imputation
		end
	end 
	return A_imputed
end
