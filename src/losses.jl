...
#     `evaluate(l::my_loss_type, u::Float64, a::Float64) ::Float64` 
#           Evaluates the function l(u,a) where u is the approximation of a
#     `grad(l::my_loss_type, u::Float64, a::Float64) ::Float64`
...
evaluate(l::quadratic, u::Float64, a::Float64) = l.scale*(u-a)^2

grad(l::quadratic, u::Float64, a::Float64) = (u-a)*l.scale
...
evaluate(l::l1, u::Float64, a::Float64) = l.scale*abs(u-a)

grad(l::l1, u::Float64, a::Float64) = sign(u-a)*l.scale
...
function evaluate(l::huber, u::Float64, a::Float64)
    abs(u-a) > l.crossover ? (abs(u-a) - l.crossover + l.crossover^2)*l.scale : (u-a)^2*l.scale
end

grad(l::huber,u::Float64,a::Float64) = abs(u-a)>l.crossover ? sign(u-a)*l.scale : (u-a)*l.scale
...
evaluate(l::periodic, u::Float64, a::Float64) = l.scale*(1-cos((a-u)*(2*pi)/l.T))

grad(l::periodic, u::Float64, a::Float64) = -l.scale*((2*pi)/l.T)*sin((a-u)*(2*pi)/l.T)
...
function evaluate(l::poisson, u::Float64, a::Float64) 
    exp(u) - a*u
end

grad(l::poisson, u::Float64, a::Float64) = exp(u) - a
...
function evaluate(l::ordinal_hinge, u::Float64, a::Float64)
    #a = round(a)
    if u > l.max-1
...
function grad(l::ordinal_hinge, u::Float64, a::Float64)
    #a = round(a)
    if u > a
        # number of levels higher than true level
...
evaluate(l::logistic, u::Float64, a::Float64) = l.scale*log(1+exp(-a*u))

grad(l::logistic, u::Float64, a::Float64) = -a*l.scale/(1+exp(a*u))
...
function evaluate(l::weighted_hinge, u::Float64, a::Float64)
...
function grad(l::weighted_hinge, u::Float64, a::Float64)
    g = (a*u>=1 ? 0 : -a*l.scale)
    if a>0
