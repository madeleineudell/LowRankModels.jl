using DataFrames, LowRankModels, Compat, Random, StatsBase
...
r = QuadReg(.1)
@compat obs = Array{Tuple{Int,Int}}(undef,0)
for i=1:m
    for j=1:n
        if B[i,j] == 1
            push!(obs, (i,j))
...
function censored_regularization_path(train_glrm::GLRM, test_glrm::GLRM; params=Params(), reg_params=exp10.(range(2,stop=-2,length=5)),
    ...
    @compat solution = Array{Tuple{Float64,Float64}}(undef, length(reg_params))
