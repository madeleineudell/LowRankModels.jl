...
# make a GLRM on the whole dataframe using mutable struct imputation
auto_glrm = GLRM(df, 3)

# now we'll try it without mutable struct imputation
# we'll just fit four of the columns, to try out all four data mutable structs
